...
class MUsuariosController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['MUsuarios'];
                        
                        $model->ContrasenaUsuario = sha1($model->ContrasenaUsuario);
			$id=Yii::app()->user->id;
	...
	public function actionIndex()
	{
            ...
            $criteria = new CDbCriteria();            
                ...
                $model=new MUsuarios('search');
                $model->unsetAttributes();  // clear any default values

                if(isset($_GET['MUsuarios']))
		{
                        $model->attributes=$_GET['MUsuarios'];
			
			
                   	
                       if (!empty($model->IdUsuario)) $criteria->addCondition("IdUsuario = '".$model->IdUsuario."'");
                     
                    	
                       if (!empty($model->Documento)) $criteria->addCondition("Documento = '".$model->Documento."'");
                     
                    	
                       if (!empty($model->LoginUsuario)) $criteria->addCondition("LoginUsuario = '".$model->LoginUsuario."'");
                     
                    	
                       if (!empty($model->ContrasenaUsuario)) $criteria->addCondition("ContrasenaUsuario = '".$model->ContrasenaUsuario."'");
                    ...
                    $session['MUsuarios_records']=MUsuarios::model()->findAll($criteria); 
		}
	...
	{
		$model=MUsuarios::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='musuarios-form')
		{
        ...
        public function actionGenerateExcel()
	{
            ...
            $session->open();		
             ...
             if(isset($session['MUsuarios_records']))
               {
                $model=$session['MUsuarios_records'];
               }
               else
                 $model = MUsuarios::model()->findAll();
        ...
        public function actionGeneratePdf() 
	{
		...
		$pdf->SetTitle('MUsuarios Report');
		$pdf->SetSubject('MUsuarios Report');
		...
		$pdf->LastPage();
		$pdf->Output("MUsuarios_002.pdf", "I");
	}
}
