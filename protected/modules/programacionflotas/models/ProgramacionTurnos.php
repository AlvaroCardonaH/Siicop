<?php

/**
 * This is the model class for table "t_programacionturnos".
 *
 * The followings are the available columns in table 't_programacionturnos':
 * @property integer $IdProgramacionTurno
 * @property integer $IdProgramacionSugerida
 * @property integer $IdFlota
 * @property integer $IdClasificacionDia
 * @property string $FechaInicial
 * @property string $FechaFinal
 *
 * The followings are the available model relations:
 * @property MClasificaciondias $idClasificacionDia
 * @property MFlotas $idFlota
 * @property TProgramacionsugerida $idProgramacionSugerida
 */
class ProgramacionTurnos extends CActiveRecord
{
	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 't_programacionturnos';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('IdProgramacionSugerida, IdFlota, IdClasificacionDia, FechaInicial, FechaFinal', 'required'),
			array('IdProgramacionSugerida, IdFlota, IdClasificacionDia', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('IdProgramacionTurno, IdProgramacionSugerida, IdFlota, IdClasificacionDia, FechaInicial, FechaFinal', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
			'idClasificacionDia' => array(self::BELONGS_TO, 'Clasificaciondias', 'IdClasificacionDia'),
			'idFlota' => array(self::BELONGS_TO, 'Flotas', 'IdFlota'),
			'idProgramacionSugerida' => array(self::BELONGS_TO, 'Programacionsugerida', 'IdProgramacionSugerida'),
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'IdProgramacionTurno' => 'Id Programacion Turno',
			'IdProgramacionSugerida' => 'Id Programacion Sugerida',
			'IdFlota' => 'Id Flota',
			'IdClasificacionDia' => 'Id Clasificacion Dia',
			'FechaInicial' => 'Fecha Inicial',
			'FechaFinal' => 'Fecha Final',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 *
	 * Typical usecase:
	 * - Initialize the model fields with values from filter form.
	 * - Execute this method to get CActiveDataProvider instance which will filter
	 * models according to data in model fields.
	 * - Pass data provider to CGridView, CListView or any similar widget.
	 *
	 * @return CActiveDataProvider the data provider that can return the models
	 * based on the search/filter conditions.
	 */
	public function search()
	{
		// @todo Please modify the following code to remove attributes that should not be searched.

		$criteria=new CDbCriteria;
               // $criteria->with = array('idClasificacionDia');


		$criteria->compare('IdProgramacionTurno',$this->IdProgramacionTurno);
		$criteria->compare('IdProgramacionSugerida',$this->IdProgramacionSugerida);
		$criteria->compare('IdFlota',$this->IdFlota);
		$criteria->compare('IdClasificacionDia',$this->IdClasificacionDia);
		$criteria->compare('FechaInicial',$this->FechaInicial,true);
		$criteria->compare('FechaFinal',$this->FechaFinal,true);
                //$criteria->group = 'IdFlota';
               //$criteria->order = 'idClasificacionDia.NombreClasificacionDia DESC';

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}

	/**
	 * Returns the static model of the specified AR class.
	 * Please note that you should have this exact method in all your CActiveRecord descendants!
	 * @param string $className active record class name.
	 * @return ProgramacionTurnos the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}
}
